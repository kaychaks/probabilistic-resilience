...
function [c_j_p_d, c_p] = compute_heuristic_cjpd(g_b, t_m_a, s_m_a, o_v, u_v) 		#Heuristic search of the ML trajectories 
	...
	#exploit "inertia" of trajectories, "similarity" of emanations
...
function r_m = recoverability_probability(max_p, max_q, c_j_p_d); 		#sum the probabilities of recoverable trajectories
	r_m = zeros(max_p, max_q);
	number_of_recoverable_trajectories = zeros(max_p, max_q);		#for the cartesian product of two ranges of parameters
	for i=1:max_p
		for j=1:max_q
			for k=1:length(c_j_p_d)
				if p_q_recoverable(i, j, c_j_p_d(k).state_trajectory)		#alternative utility_trajectory
					...
					number_of_recoverable_trajectories(i,j)++;
				endif
			endfor
		endfor
	endfor	
	number_of_recoverable_trajectories
endfunction

function print_recoverability_matrix(r_m)
	for i=1:rows(r_m)
		for j=1:columns(r_m)
			printf("<%d,%d>-recoverability, %f\n", i, j, r_m(i,j));
		endfor
	endfor
endfunction
...
function f_v = functionality_probability(max_f, c_j_p_d)			#sum the probabilities of functional trajectories
	...
	number_of_functional_trajectories
endfunction

function print_functionality_vector(f_v);	
	for i=1:length(f_v)
		printf("%d-functionality, %f \n", i, f_v(1,i));
	endfor
endfunction
		...
		for j=1:max_p
			for k=1:max_q
				for l=1:max_f
					...
					number_of_trajectories_in_which_the_property_holds = 0;
					for m=1:length(c_j_p_d)
						if l_p_q_f_resilient(i,j,k,l,c_j_p_d(m).state_trajectory)	
							...
							number_of_trajectories_in_which_the_property_holds++;
						endif
					endfor
					index++;
					number_of_trajectories_in_which_the_property_holds;
...
function print_resilience_table(r_t)
	for i=1:rows(r_t)
			printf("<%d,%d,%d,%d>-resilience, %f\n", r_t(i,1), r_t(i,2), r_t(i,3), r_t(i,4), r_t(i,5));
	endfor
endfunction
...
for iter=1:experiments
	...
	#p = compute_l_resistance(ground_belief, transition_model_array, sensor_model_array, disambiguated_observation_vector, utility_vector, 1)
	
	printf("\n");
	max_functionality_parameter = columns(transition_model_array(1).val);				#alternative: max(utility_vector);
	functionality_vector = functionality_probability(max_functionality_parameter, sorted_conditional_joint_probability_distribution);
	print_functionality_vector(functionality_vector);
	...
	print_recoverability_matrix(recoverability_matrix);
	...
	print_resilience_table(resilience_table);
